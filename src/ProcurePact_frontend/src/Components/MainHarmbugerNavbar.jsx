...
import "../styles/MainHarmburgerNavbar.css";
...
const MainHarmburgerNavbar = () => {
  ...
  const handleAuth = async () => {
    ...
    if (isAuthenticated) {
      await authClient.logout();
      await onIdentityUpdate();
      // disconnectWallet(); // implement if needed
      navigate("/");
    } else {
      // login(); // implement if needed
    }
  };

  return (
    <nav className="harmburger-navbar">
      <div className="navbar-header">
        <FaBars className="hamburger-icon" onClick={() => setOpen(true)} />
        <span className="navbar-title">ProcurePact</span>
      </div>
      {open && (
...
export default MainHarmburgerNavbar;
